...
%% Pid ! .... ///// globalis, nincs hiba
%% {name, node} ! ...  //////globalis, nincs hiba
%% name ! ... //// lokalis, hibat adhat, ha name nem letezik

% ELofeltetel: node kapcsolat, pl net_adm:ping(...)
-define(JS, jobserver).
%-define(JS, global:whereis_name(jobserver)).
% JS ! ...
% 
evaluate(M, F, A) ->
    global:send(?JS, {job, {M, F, A}, Ref=make_ref()}),
    Ref.

result(Ref) ->
    global:send(?JS, {result, Ref, self()}),
    receive
        % {A, Ref} -> A
        {failed, Ref} -> failed;
...
start(Max) ->
    global:register_name(jobserver, spawn_link(?MODULE, init, [Max])).
