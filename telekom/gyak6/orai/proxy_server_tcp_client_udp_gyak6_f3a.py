...
class CalculatorTCPSelectServer:
  def __init__(self, addr='localhost', port=10001, timeout=1, calcServerHost = 'localhost', calcServerPort=10002):
    self.server = self.setupServer(addr, port)
    self.calcClient = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    self.calcServerAddr = (calcServerHost, calcServerPort)
  ...
  def handleDataFromClient(self, sock):
        ...
        data = data.strip()
        if data:
            self.calcClient.sendto(data,self.calcServerAddr)
            result, _ = self.calcClient.recvfrom(1024)
            sock.sendall(result)
