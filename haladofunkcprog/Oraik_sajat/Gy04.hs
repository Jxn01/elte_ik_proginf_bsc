...
instance Foldable SparseList where
  ...
  foldr f z (Cons a as) = let rest = foldr f z as in f a rest
...
filterMaybe f [] = Just []
filterMaybe f (a:as)
    | f a == Just True = Just (a : fromMaybe [] (filterMaybe f as))
    | f a == Just False = Just (fromMaybe [] (filterMaybe f as))
    | f a == Nothing = Nothing
    | otherwise = undefined
...
mapMaybeTree f (Node l r) = undefined
