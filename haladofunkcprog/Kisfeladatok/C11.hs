...
pLessT :: Parser Exp
pLessT = nonAssoc LessT pPlus (char' '<')

pEqual :: Parser Exp
pEqual = nonAssoc Equal pLessT (string' "==")

pExp :: Parser Exp
pExp = topLevel pEqual

evalExp :: Exp -> Int
evalExp (Lit i) = i
evalExp (Plus l r) = evalExp l + evalExp r
evalExp (Mult l r) = evalExp l * evalExp r
evalExp (LessT l r) = if evalExp l < evalExp r then 1 else 0
evalExp (Equal l r) = if evalExp l == evalExp r then 1 else 0
